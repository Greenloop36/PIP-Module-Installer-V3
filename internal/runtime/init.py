...
def main() -> bool | None:
    ...
    for i in RequiredModules:
        ...
        PACKAGE_IMPORT_AS: str = None

        if isinstance(i, dict):
            PACKAGE_NAME = i["PackageName"]
            PACKAGE_IMPORT_AS = i["ImportAs"]
        else: PACKAGE_NAME = PACKAGE_IMPORT_AS = i

        print(f"\t| \"{PACKAGE_IMPORT_AS}\":", end = " ", flush=True)
        Installed = IsModuleInstalled(PACKAGE_IMPORT_AS)

        if not Installed:
            ModulesToInstall.append(PACKAGE_NAME)
